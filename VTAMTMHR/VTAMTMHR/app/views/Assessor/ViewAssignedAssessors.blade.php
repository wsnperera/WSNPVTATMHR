	...
	<div class="span12">
       <div class="span8" style="width: 100%">
            <div class="page-header position-relative">
                ...
                <h1>Assessor Details<small><i class="icon-double-angle-right"></i>View</small></h1>
            </div>
		 </div>	
            <form class="form-horizontal" action='{{url("ExamViewNPrintLettersForAssignedAssessors")}}' method="POST"  id='NewModule'>
		 ...
		 <div class="span12">
            <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                ...
                <tr>
						...
						<th class="center">Print Leeters(For Original Assesors</th>
						<th class="center">Print Leeters(For Renpminated Assesors</th>
						...
						<th class="center">Assessor Entered Dates</th>
                ...
                @if(isset($trplans))
                    ...
                    <tr>
					   ...
					   <td class="center">
                                ...
                                <form id="editform"  action='ExamAssignAssessorForCourse' method="GET" >
                                    <input type="hidden" name='edit_id' value='{{$mm->id}}' />
									<input type="hidden" value="{{$CenterIDD}}" name="CenterIDD" id="CenterIDD"/>
									<input type="hidden" value="{{$YearD}}" name="YearD" id="YearD"/>
									<input type="hidden" value="{{$BatchD}}" name="BatchD" id="BatchD"/>
									<input type="hidden" value="{{$districtD}}" name="districtD" id="districtD"/>
                                    <button type="Submit" class="btn btn-primary btn-mini"><i class="icon-edit icon-2x icon-only"></i></button>
                                </form>
                       
                    </td>
					
                     
						<td class='center'> <font color="Green"><a class="Green"  id="{{$mm->id}}"> <i class="icon-print bigger-200"></i></a> </font>
                       
                    </td>
					<td class='center'> 
					@if($mm->AssessorReNominated == '1')
					<font color="PINK"><a class="PINK"  id="{{$mm->id}}"> <i class="icon-print bigger-200"></i></a> </font>
                    @else
					<font color="red">Not Renominated</font>	
					@endif
                    </td>
            ...
            $.ajax
                    ({
                        beforeSend: function()
                        {
                            ...
                            document.getElementById('img4').innerHTML = "<img src=\"{{Url('assets/images/redballs.gif')}}\"/>";
                        },
                        type: "POST",
                        url: "{{Url('ExamPrintAssessorAssignedLetterForRenominated')}}",
                        data: {CS_ID: CS_ID},
      ...
      // alert(CS_ID);
            ...
            $.ajax
                    ({
                        ...
                        type: "POST",
                        url: "{{Url('ExamPrintAssessorAssignedLetter')}}",
                        data: {CS_ID: CS_ID},
                        success:function response(responseText, statusText, xhr, $form)
                        {
