...
@if(isset($Issearch))
<a href={{url('ViewHRALAttempt')}}> << Back to A/L Attempts</a> 
@endif
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>A/L Attempts<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHRALAttempt'))
             <form name='search' action="{{url('CreateHRALAttempt')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create  A/L Attempt</button>
			 </form>
	    @endif
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
					    <td>{{$mc->Attempt}}</td>
                      <td>
					   @if($user->hasPermission('EditHRALAttempt'))
						   <form id="deleteform"  action='EditHRALAttempt' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHRALAttempt'))
							 <form id="deleteform"  action='DeleteHRALAttempt' method="POST" onsubmit="return doConfirm('{{$mc->Attempt}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
