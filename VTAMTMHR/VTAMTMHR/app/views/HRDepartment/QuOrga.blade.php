...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>Department<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHRDepartment'))
             <form name='search' action="{{url('CreateHRDepartment')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Department</button>
			 </form>
	    @endif
				...
				<thead>
                 ...
                 <tbody>
                    ...
                    <tr>
                      ...
                      <td>
					   @if($user->hasPermission('EditHRDepartment'))
						   <form id="deleteform"  action='EditHRDepartment' method="GET">
                                <input type="hidden" name='id' value="{{$mc->D_ID}}" />
                                <button type="submit" class="btn btn-success btn-small"><i class="icon-pencil icon-1x icon-only"></i></button>
                            </form>
					   @endif
						</td>
                        <td>
						@if($user->hasPermission('DeleteHRDepartment'))
							 <form id="deleteform"  action='DeleteHRDepartment' method="POST" onsubmit="return doConfirm('{{$mc->DepartmentName}}',this)">
                                <input type="hidden" name='id' value="{{$mc->D_ID}}" />
