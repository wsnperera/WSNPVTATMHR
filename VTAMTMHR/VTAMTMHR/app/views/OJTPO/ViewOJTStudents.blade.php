...
<div class="page-content">
	...
	<div class="span12">
        <div class="span8" style="width: 100%">
            <div class="page-header position-relative">
                ...
                <h1>View OJT Students<small><i class="icon-double-angle-right"></i>PO View</small></h1>
            </div>
			 </div>
            <form class="form-horizontal" action='{{url("ViewMyOJTStudents")}}' method="POST"  id='NewModule'>
    ...
    <tr>
        ...
        <td>
            <form name='search' action="{{url('PrintOJTPlacedMyStudents')}}" method='POST' class="form-horizontal">
		 ...
		 <div class="span12">
            <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                ...
                <tr>
						...
						<th class="center">Mark OJT Completion</th>
						<th class="center">Enter Monitoring Details</th>
						<th class="center">View Monitoring Details</th>
						...
						<th class="center">Salary Per Month</th>
                ...
                @if(isset($trplans))
                  ...
                  <tr>
					   ...
					   <td class='center'> 
					   @if($user->hasPermission('MarkOJTCompletion'))
						   @if($mm->OJTDropout == 0)
					          <font color="green"><a class="green"  id="{{$mm->ojtplaceiD}}"> <i class="icon-thumbs-up bigger-200"></i></a> </font>
						  @else
							<font color="red">This Student Already Dropout During the OJT</font>  
				          @endif
				       @endif
					   </td>
					<td class="center">
						   ...
						   @if($mm->OJTDropout == 0)
                                <form id="editform"  action='IRCreatePOMonitoringData' method="GET" >
                                    <input type="hidden" name='edit_id' id='edit_id' value='{{$mm->ojtplaceiD}}' />
									<input type="hidden" value="{{$YearD}}" name="YearD" id="YearD"/>
									<input type="hidden" value="{{$BatchD}}" name="BatchD" id="BatchD"/>
					   ...
					   <?php
						$Count = IRPOMonitoring::where('ojtplacementID','=',$mm->ojtplaceiD)->where('Deleted','=',0)->where('POEmpId','=',User::getSysUser()->EmpId)->count('id');
						?>
                       <td class='center'>  <font color="pink"><a class="pink"  id="{{$mm->ojtplaceiD}}"> <i class="badge badge-warning"><h4>{{$Count}}</h4></i></a> </font></td>
						...
						<td>{{$mm->CompanyName}}</td>
						<td>{{$mm->CAddress}}</td>
                        <td>{{$mm->Category}}</td>
						<td>{{$mm->StartingDate}}</td>
						<td>{{$mm->EndDate}}</td>
						<td>{{$mm->Salary}}</td>
						<td class="center">
...
<script>
	...
	$(".pink").click(function(){
     ...
     $.ajax({
                    url: "{{url::to('IRGetMonitoringPOList')}}",
                    data: {id: id},
                     dataType: "json", 
                   success: function(result) {
					    var c=1;
						var x = '<form id="infos" action=""><div class="control-group">'
						  ...
						  +'<th>Date Visited</th>'
						  +'<th>Visited by</th>'
						  +'<th>Trainee Attendence</th>'
						  +'<th>Trainee Satisfaction</th>'
						  +'</tr></thead><tbody></form>';
                         $.each(result, function(i, item)
                        {
							var tad = '';
							var tsat = '';
							if(item.TraineeAttendence == 1)
							{
								tad = 'Present';
							}
							else
							...
							if(item.TraineeSatisfaction == 1)
							{
								tsat = 'Very Satisfied';
								
							}
							else if(item.TraineeSatisfaction == 2)
							{
								tsat = 'Satisfied';
							}
							else if(item.TraineeSatisfaction == 3)
							{
								tsat = 'Neutral';
							}
							else if(item.TraineeSatisfaction == 4)
							{
								tsat = 'Dissatisfied';
							}
							else if(item.TraineeSatisfaction == 5)
							{
								tsat = 'Very Dissatisfied';
							}
							else
							{
								tsat = 'None';
