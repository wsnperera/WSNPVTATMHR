...
<div class="page-content">
    ...
    <div class="row-fluid">
       @if($user->hasPermission('CreateHREmployeeALResults'))
             <form name='search' action="{{url('CreateHREmployeeALResults')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-pink">
                <i class="icon-pencil bigger-100"></i>Create Employee A/L Results</button>
			 </form>
	    @endif
            ...
            <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                <thead>
                    <tr>
						...
						<th rowspan="2"> @if($user->hasPermission('EditHREmployeeALResults')) Edit Results @endif</th>
                        <th rowspan="2">@if($user->hasPermission('DeleteHREmployeeALResults')) Remove  @endif </th>
                    </tr>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                       ...
                       <td>
					   @if($user->hasPermission('EditHREmployeeALResults'))
						   <form id="deleteform"  action='EditHREmployeeALResults' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHREmployeeALResults'))
							 <form id="deleteform"  action='DeleteHREmployeeALResults' method="POST" onsubmit="return doConfirm('{{$eq->Attempt}}- {{$eq->Year}}- {{$eq->NIC}}',this)">
                                <input type="hidden" name='id' value="{{$eq->id}}" />
...
<script type="text/javascript">
                                                ...
                                                bootbox.confirm("Are you sure you want to remove A/L results of the Employee Attempt " + empqua, function(result) {
                                                if (result) {
                                                formobj.submit();
                                                }
                                                });
...
$(".pink").click(function(){
    ...
    // alert(id);
     ...
     $.ajax({
                    url: "{{url::to('HREmployeeALResultsSheet')}}",
                    data: {id: id},
                     dataType: "json", 
                   success: function(result) {
					   var c=1;
					   var msg = 'General Knowledge';
