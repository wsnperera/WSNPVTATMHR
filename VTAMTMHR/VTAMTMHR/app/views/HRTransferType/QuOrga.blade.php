...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>Transfer Types<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHRTransferType'))
             <form name='search' action="{{url('CreateHRTransferType')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Transfer Type</button>
			 </form>
	    @endif
             ...
             <table id="sample-table-2" class="table table-striped table-bordered table-hover">
				...
				<thead>
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
					    <td>{{$mc->TransferType}}</td>
						<td>@if($mc->Available == 1) Yes @else No @endif</td>
                      <td>
					   @if($user->hasPermission('EditHRTransferType'))
						   <form id="deleteform"  action='EditHRTransferType' method="GET">
                                <input type="hidden" name='id' value="{{$mc->T_ID}}" />
                                <button type="submit" class="btn btn-success btn-small"><i class="icon-pencil icon-1x icon-only"></i></button>
                            </form>
					   @endif
						</td>
                        <td>
						@if($user->hasPermission('DeleteHRTransferType'))
							 <form id="deleteform"  action='DeleteHRTransferType' method="POST" onsubmit="return doConfirm('{{$mc->TransferType}}',this)">
                                <input type="hidden" name='id' value="{{$mc->T_ID}}" />
