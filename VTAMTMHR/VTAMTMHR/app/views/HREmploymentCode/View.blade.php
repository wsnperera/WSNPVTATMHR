...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>Employment Code<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHrEmploymentCode'))
             <form name='search' action="{{url('CreateHrEmploymentCode')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Employment Code</button>
			 </form>
	    @endif
		<hr/>
        <div class="span12">
		<form name='search' action="{{url('SearchHrEmploymentCode')}}" method='get' class="form-horizontal">
				...
				<thead>
                 ...
                 <tbody>
                    ...
                    <tr>
                       ...
                       <td>{{$mc->SalaryCode}}</td>
                       <td>{{$mc->SalaryScale}}</td>
					   <td>{{$mc->ArrivalTime}}</td>
					   <td>{{$mc->Departute}}</td>
                       <td class='center'>
					   @if($mc->Active == '1')
				   ...
				   @endif
                       ...
                       <td>
					   @if($user->hasPermission('EditHrEmploymentCode'))
						   <form id="deleteform"  action='EditHrEmploymentCode' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHrEmploymentCode'))
							 <form id="deleteform"  action='DeleteHrEmploymentCode' method="POST" onsubmit="return doConfirm('{{$mc->SalaryCode}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
