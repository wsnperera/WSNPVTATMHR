...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>Qualification <small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHRQualification'))
             <form name='search' action="{{url('CreateHRQualification')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Qualification </button>
			 </form>
	    @endif
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
					    <td>{{$mc->Type}}</td>
						<td>{{$mc->QCategory}}</td>
						<td>{{$mc->Qualification}}</td>
                      <td>
					   @if($user->hasPermission('EditHRQualification'))
						   <form id="deleteform"  action='EditHRQualification' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHRQualification'))
							 <form id="deleteform"  action='DeleteHRQualification' method="POST" onsubmit="return doConfirm('{{$mc->Qualification}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
