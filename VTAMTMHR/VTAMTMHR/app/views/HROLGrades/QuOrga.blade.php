...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>O/L & A/L Grades<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHROLGrades'))
             <form name='search' action="{{url('CreateHROLGrades')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Grade</button>
			 </form>
	    @endif
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
					    <td>{{$mc->Grade}}</td>
						 <td>{{$mc->GradeName}}</td>
						 <td>@if($mc->PassStatus == 1)Pass @else Fail @endif</td>
                      <td>
					   @if($user->hasPermission('EditHROLGrades'))
						   <form id="deleteform"  action='EditHROLGrades' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHROLGrades'))
							 <form id="deleteform"  action='DeleteHROLGrades' method="POST" onsubmit="return doConfirm('{{$mc->Grade}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
