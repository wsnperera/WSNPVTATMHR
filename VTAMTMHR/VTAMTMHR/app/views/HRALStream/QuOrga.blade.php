...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>A/L Stream<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHRALStream'))
             <form name='search' action="{{url('CreateHRALStream')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create A/L Stream</button>
			 </form>
	    @endif
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
					    <td>{{$mc->Stream}}</td>
                      <td>
					   @if($user->hasPermission('EditHRALStream'))
						   <form id="deleteform"  action='EditHRALStream' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHRALStream'))
							 <form id="deleteform"  action='DeleteHRALStream' method="POST" onsubmit="return doConfirm('{{$mc->Subject}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
