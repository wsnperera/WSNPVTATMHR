...
@if(isset($Issearch))
<a href={{url('ViewHRALSubject')}}> << Back to A/L Subjects</a> 
@endif
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>A/L Subjects<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHRALSubject'))
             <form name='search' action="{{url('CreateHRALSubject')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create A/L Subject</button>
			 </form>
	    @endif
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
					    <td>{{$mc->Subject}}</td>
                      <td>
					   @if($user->hasPermission('EditHRALSubject'))
						   <form id="deleteform"  action='EditHRALSubject' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHRALSubject'))
							 <form id="deleteform"  action='DeleteHRALSubject' method="POST" onsubmit="return doConfirm('{{$mc->Subject}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
