...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>O/L Subjects<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHROLSubject'))
             <form name='search' action="{{url('CreateHROLSubject')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Subject</button>
			 </form>
	    @endif
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
					    <td>{{$mc->Subject}}</td>
                      <td>
					   @if($user->hasPermission('EditHROLSubject'))
						   <form id="deleteform"  action='EditHROLSubject' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHROLSubject'))
							 <form id="deleteform"  action='DeleteHROLSubject' method="POST" onsubmit="return doConfirm('{{$mc->Subject}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
