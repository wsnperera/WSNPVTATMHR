...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>Service Category Wise Salary Conversions<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHrServiceCategorySalaryConversion'))
             <form name='search' action="{{url('CreateHrServiceCategorySalaryConversion')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Salary Conversion</button>
			 </form>
	    @endif
		<hr/>
        <div class="span12">
		<form name='search' action="{{url('SearchHrServiceCategorySalaryConversion')}}" method='get' class="form-horizontal">
				...
				<tr>
						<td>
							...
							<form> 
											...
											<input type="hidden" value="{{$YearP}}" name="YearP" id="YearP"/>
						...
						<!--<td>
							<form>
								...
								<i class="icon-remove bigger-200"></i>Deactivate {{$YearP}}</button><span id='img4'></span>
				...
				<thead>
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
                       <td>{{$mc->Year}}</td>
                       <td>{{$mc->ServiceCategory}}</td>
                       <td>{{$mc->SalaryCode}}</td>
                       <td>{{$mc->Grade}}</td>
					   <td>{{$mc->SalaryConversionDate}}</td>
					   <td>{{$mc->StepNo}}</td>
					   <td>{{$mc->BasicSalary}}</td>
					    <td>{{$mc->GrossSalary}}</td>
					   <td>{{$mc->AdjusmentAllowence}}</td>
					   <td>
					   @if($mc->Active == '1')
					   ...
					   <td>
					   @if($user->hasPermission('EditHrServiceCategorySalaryConversion'))
						   <form id="deleteform"  action='EditHrServiceCategorySalaryConversion' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHrServiceCategorySalaryConversion'))
							 <form id="deleteform"  action='DeleteHrServiceCategorySalaryConversion' method="POST" onsubmit="return doConfirm('{{$mc->BasicSalary}} - {{$mc->AdjusmentAllowence}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
      ...
      //alert(CD_ID); 
            ...
            $.ajax
                    ({
                        ...
                        type: "POST",
                        url: "{{Url('PrintHrServiceCategorySalaryConversion')}}",
                        data: {YearP: YearP},
