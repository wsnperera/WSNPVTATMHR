...
<div class="page-content">
    <div class="row-fluid">
		...
		@if($user->hasPermission('CreateHrServiceCategory'))
             <form name='search' action="{{url('CreateHrServiceCategory')}}" method='get'>
		...
		<hr/>
        <div class="span12">
		<form name='search' action="{{url('SearchHrServiceCategory')}}" method='get' class="form-horizontal">
				...
				<thead>
                 ...
                 <tbody>
                    ...
                    <tr>
					  ...
					  // $checked11 = HRSalaryScaleGrade::where('Deleted','=',0)->where('ServiceCategoryID','=',$mc->id)->where('Active','=',1)->orderBy->count();
					   ...
					   $CheckedGrade11 = DB::table('hrsalaryscalegrade')
						...
						->get();
					   ?>
					   @foreach($CheckedGrade11 as $cg)
					   <span>{{$cg->Grade}}</span><br/>
					   @endforeach
					   </td>
                       <td class='center'>
					   @if($mc->Active == '1')
				   ...
				   @endif
                       ...
                       <td>
					   @if($user->hasPermission('EditHrServiceCategory'))
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHrServiceCategory'))
							 <form id="deleteform"  action='DeleteHrServiceCategory' method="POST" onsubmit="return doConfirm('{{$mc->SalaryCode}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
      ...
      //alert(CD_ID); 
            ...
            $.ajax
                    ({
                        ...
                        type: "POST",
                        url: "{{Url('PrintHrServiceCategory')}}",
                        data: {YearP: YearP},
