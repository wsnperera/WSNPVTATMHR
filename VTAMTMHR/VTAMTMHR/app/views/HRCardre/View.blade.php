...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>Cardre<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateCardreDetails'))
             <form name='search' action="{{url('CreateCardreDetails')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Cardre</button>
			 </form>
	    @endif
		<hr/>
        <div class="span12">
				...
				<thead>
                 ...
                 <tbody>
                    ...
                    <tr>
                       ...
                       <td>{{$mc->DesignationCode}}</td>
					   <td>{{$mc->MaxNoOPossitions}}</td>
                       <td>{{$mc->Academic}}</td>
					   <td>{{$mc->ArrivalTime}} - {{$mc->Departute}}</td>
                       <td class='center'>
					   @if($mc->Active == '1')
                       ...
                       <td>
					   @if($user->hasPermission('EditCardreDetails'))
						   <form id="deleteform"  action='EditCardreDetails' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteCardreDetails'))
							 <form id="deleteform"  action='DeleteCardreDetails' method="POST" onsubmit="return doConfirm('{{$mc->Designation}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
