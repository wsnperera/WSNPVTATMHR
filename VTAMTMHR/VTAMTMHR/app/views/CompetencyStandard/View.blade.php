...
<div class="page-content">
    <div class="row-fluid">
        ...
        <form name='search' action="{{url('CreateCompetemcyStandard')}}" method='get'>
			...
			<?php $i = 1; ?>
                @if(isset ($NVQcompetencystandard))
					...
					<td><form id="editform"  action='editCompetemcyStandard' method="GET" >
						...
						<td>{{$m->name}}</td>
                        <td>
                            <form id="deleteform"  action='deleteCompetemcyStandard' method="POST" onsubmit="return doConfirm('{{$m->code}}',this)">
                                <input type="hidden" name='id' value="{{$m->id}}" />
