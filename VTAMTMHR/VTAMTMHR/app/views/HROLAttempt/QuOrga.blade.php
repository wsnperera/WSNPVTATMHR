...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>O/L Attempts<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHROLAttempt'))
             <form name='search' action="{{url('CreateHROLAttempt')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Attempt</button>
			 </form>
	    @endif
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
					    <td>{{$mc->Attempt}}</td>
                      <td>
					   @if($user->hasPermission('EditHROLAttempt'))
						   <form id="deleteform"  action='EditHROLAttempt' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHROLAttempt'))
							 <form id="deleteform"  action='DeleteHROLAttempt' method="POST" onsubmit="return doConfirm('{{$mc->Attempt}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
