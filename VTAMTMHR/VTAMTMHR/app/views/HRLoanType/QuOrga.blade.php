...
@if(isset($Issearch))
<a href={{url('ViewHRLoanType')}}> << Back to Loan Types</a> 
@endif
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>Loan Types<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHRLoanType'))
             <form name='search' action="{{url('CreateHRLoanType')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Loan Type</button>
			 </form>
	    @endif
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$i++}}</td> 
					    <td>{{$mc->LoanType}}</td>
                      <td>
					   @if($user->hasPermission('EditHRLoanType'))
						   <form id="deleteform"  action='EditHRLoanType' method="GET">
					   ...
					   @endif
                        ...
                        <td>
						@if($user->hasPermission('DeleteHRLoanType'))
							 <form id="deleteform"  action='DeleteHRLoanType' method="POST" onsubmit="return doConfirm('{{$mc->LoanType}}',this)">
                                <input type="hidden" name='id' value="{{$mc->id}}" />
