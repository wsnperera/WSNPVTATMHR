...
<div class="page-content">
    <div class="row-fluid">
        <div class="page-header position-relative">
            <h1>Employee Types<small><i class="icon-double-angle-right"></i>View</small></h1>
        </div>
		@if($user->hasPermission('CreateHREmployeeType'))
             <form name='search' action="{{url('CreateHREmployeeType')}}" method='get'>
				<button type="submit" id="submit" class="btn btn-primary">
                <i class="icon-pencil bigger-100"></i>Create Employee Type</button>
			 </form>
	    @endif
				...
				<thead>
                 ...
                 <tbody>
                    ...
                    <tr>
                      ...
                      <td>
					   @if($user->hasPermission('EditHREmployeeType'))
						   <form id="deleteform"  action='EditHREmployeeType' method="GET">
                                <input type="hidden" name='id' value="{{$mc->ET_ID}}" />
                                <button type="submit" class="btn btn-success btn-small"><i class="icon-pencil icon-1x icon-only"></i></button>
                            </form>
					   @endif
						</td>
                        <td>
						@if($user->hasPermission('DeleteHREmployeeType'))
							 <form id="deleteform"  action='DeleteHREmployeeType' method="POST" onsubmit="return doConfirm('{{$mc->EmployeeType}}',this)">
                                <input type="hidden" name='id' value="{{$mc->ET_ID}}" />
